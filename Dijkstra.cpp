...
// Allocates memory for adjacency list or Initialisation
Graph::Graph(int V) 
{ 
	this->V = V; 
...
{ 
	// Create a set to store vertices that are visited 
	...
	cout<<"\nVertex Distance from "<<src<<" to "<<des<<" is :: "<<dist[des];
	cout<<"\nVertex Distances to other vertexes are \n";
	for (int i = 0; i < V; ++i) 
	{
		if(i==des)
		continue;
...
{ 
	...
	// making graph, it can be taken as input also
	g.addEdge(0, 1, 4); 
	g.addEdge(0, 7, 8); 
	g.addEdge(1, 2, 8); 
	g.addEdge(1, 7, 11); 
	...
	g.shortestPath(0,5); // 0 is given as the source
